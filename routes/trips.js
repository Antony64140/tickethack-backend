...
router.post('/selectrip', async (req, res) => {
  try {
    ...
    const TripExist = await trip.findOne({
      departure: { $regex: `^${depart}$`, $options: 'i' },
      arrival: { $regex: `^${arrivee}$`, $options: 'i' },
      price: Number(prix),
      date: jour,
    });
    ...
    if (TripExist) {
      return res.json({ result: false, error: 'Ce trajet existe déjà' });
    }
...
router.get('/panier', async function (req, res, next) {
  try {
    const TripPanier = await trip.find({ etat: 0 });
    return res.status(200).json({ result: true, trips: TripPanier });
  } catch (err) {
    console.error(err);
    res
      .status(500)
      .json({ result: false, error: 'RECUP PANIER Database error' });
  }
});

router.get('/book', async function (req, res, next) {
  try {
    const TripPanier = await trip.find({ etat: 1 });
    return res.status(200).json({ result: true, trips: TripPanier });
  } catch (err) {
    console.error(err);
...
router.delete('/suppr/:id', async (req, res) => {
  try {
